...
const AdminPanel = ({ currentUser }) => {
  ...
  const filteredPhotos = photos.filter(photo => {
    ...
    if (searchTerm) {
      ...
      const matchesSearch = 
        ...
        (photo.userEmail && photo.userEmail.toLowerCase().includes(searchLower)) ||
  ...
  const handleDeletePhoto = async (photoId) => {
    ...
    try {
      ...
      setError('');
      // Photo will be removed from state automatically via real-time listener
    } catch (error) {
            ...
            {filteredPhotos.map((photo) => (
              ...
              >
                ...
                <div style={{ position: 'relative' }}>
                  <img
                    src={photo.imageUrl || photo.photoURL}
                    alt="User photo"
                    style={{
                      width: '100%',
                      height: '200px',
